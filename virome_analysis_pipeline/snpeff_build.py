...
class SnpEffBuilder:
    ...
    def build_from_ncbi(self, db_name, accessions):
        ...
        # 病毒基因组几乎 100% 触发 "Protein verification failures"
        # -noCheckProtein -noCheckCds 避免病毒基因组构建 SnpEff DB 时失败
        cmd = ["java", f"-Xmx{self.memory}", "-jar", self.jar, "build",
               "-genbank", "-v", "-noCheckProtein", "-noCheckCds",
