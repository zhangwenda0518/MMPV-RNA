...
class VirusQuantificationPipeline:
    ...
    def _compute_ani_pi_worker(self, task):
        ...
        if not os.path.exists(bam_path): return (sname, ref, 0.0, 0.0)
        ...
        try:
            with pysam.AlignmentFile(bam_path, "rb") as bam:
                if ref not in bam.references: return (sname, ref, 0.0, 0.0)
                
                # 【修复1】：安全获取总读数
                total_reads = _get_ref_read_count_safe(bam, ref)
                if total_reads == 0: return (sname, ref, 0.0, 0.0)
            ...
            return (sname, ref, 0.0, 0.0)
            
        avg_ani = (ani_sum / ani_cnt) * 100.0 if ani_cnt > 0 else 0.0
        ...
        avg_pi = (pi_sum / covered_pos) if covered_pos > 0 else 0.0
        return (sname, ref, round(avg_ani, 2), round(avg_pi, 5))
    ...
    def summarize_results_polars(self, all_metrics):
        ...
        asm_df = df.group_by(["Sample", "taxid"]).agg([
            ...
            pl.col("MeanDepth").mean().alias("Avg_MeanDepth"), 
        ...
        asm_df = asm_df.with_columns([
            ...
            (pl.col("Asm_EM_Reads") * 1e9 / (pl.col("Sample_Total_Mapped") * pl.col("Asm_Length"))).round(2).alias("Asm_FPKM"),
        ...
        final_df = asm_df.filter(
            (pl.col("Avg_Coverage(%)") >= self.args.coverage) & (pl.col("Avg_MeanDepth") >= self.args.meandepth) &
            (pl.col("Uniq_Reads") >= self.args.min_uniq_reads) & (pl.col("Asm_TPM") >= self.args.min_tpm)
        )
        final_df.write_csv(str(self.output_dir / "summary" / f"all_viruses.summary.{ext}"), separator=sep)
        
        pre_best_df = final_df.sort(["Sample", "taxid", "Avg_Coverage(%)"], descending=[False, False, True]).unique(subset=["Sample", "taxid"], keep="first", maintain_order=True)
        ...
        with ProcessPoolExecutor(max_workers=self.args.threads) as executor:
            ...
            for future in tqdm(as_completed(futures), total=len(futures), desc="计算 ANI/Pi"):
                try:
                    ani_pi_results.append(future.result())
                except Exception as e:
                    logger.warning(f"ANI/Pi 计算异常: {e}")
                    ani_pi_results.append((None, None, 0.0, 0.0))
        ...
        if len(ani_pi_results) == 0:
            ...
            cleaned = []
            for row in ani_pi_results:
                if not row: continue
                if isinstance(row, (list, tuple)):
                    r_dict = {"Sample": row[0], "Rep_Accession": row[1]}
                    if len(row) == 4:
                        r_dict["Avg_Read_ANI"] = row[2]
                        r_dict["Pi"] = row[3]
                    elif len(row) >= 5:
                        r_dict["SNPs"] = row[2]
                        r_dict["Pi"] = row[3]
                        r_dict["Avg_Read_ANI"] = row[4]
                ...
                for k, v in r_dict.items():
                    if k == "SNPs":
                        try: clean_row[k] = int(float(v))
                        except: clean_row[k] = None
                    elif k in ["Pi", "Avg_Read_ANI", "Avg_Pi"]:
                        k_new = "Pi" if k == "Avg_Pi" else k
                        try: clean_row[k_new] = float(v)
                        except: clean_row[k_new] = None
                    else:
                        clean_row[k] = str(v)
                cleaned.append(clean_row)
            ap_df = pl.DataFrame(cleaned)
        #ap_df = pl.DataFrame(ani_pi_results, schema=["Sample", "Rep_Accession", "Avg_Read_ANI", "Avg_Pi"], orient="row")
        
        merged_df = pre_best_df.join(ap_df, on=["Sample", "Rep_Accession"], how="left")
        if "Pi" in merged_df.columns and "Avg_Pi" not in merged_df.columns:
            merged_df = merged_df.with_columns(pl.col("Pi").alias("Avg_Pi"))
        elif "Avg_Pi" in merged_df.columns and "Pi" not in merged_df.columns:
            merged_df = merged_df.with_columns(pl.col("Avg_Pi").alias("Pi"))
        ...
        final_cols = [
            ...
            "Avg_Coverage(%)", "Avg_MeanDepth", "Avg_Read_ANI", "Avg_Pi", 
        ...
        with open(self.output_dir / 'summary' / 'analysis_report.txt', 'w') as f:
            ...
            if len(best_df) > 0:
                ...
                for row in asm_counts.iter_rows(named=True):
                    asm, count = row['Adjusted_Species'], row['count' if 'count' in asm_counts.columns else 'counts']
                    ...
                    if len(vd) > 0:
                        ...
                        f.write(f"   ├─ 测定 ANI: {(vd['Avg_Read_ANI'].mean() or 0.0):.2f}% | 多态性 Pi: {(vd['Pi'].mean() or 0.0):.5f}\n")
...
def main():
    parser = argparse.ArgumentParser(description='宏病毒鉴定与精确定量管线 V34')
    ...
    parser.add_argument('-o', '--output_dir', default='./virus_out', help='输出目录')
    ...
    parser.add_argument('-t', '--threads', type=int, default=8, help='并行处理的样本数')
    parser.add_argument('--align_threads', type=int, default=4, help='单样本比对线程')
