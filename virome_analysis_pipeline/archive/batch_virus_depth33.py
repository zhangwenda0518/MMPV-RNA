...
6. [单双端适配] 自动判别单端/双端数据，输出匹配格式的 FASTQ。
7. [进度透明] 详细日志输出比对、过滤、深度、EM分配全过程。
================================================================================
"""
...
class VirusQuantificationPipeline:
    ...
    def detect_paired_mode(self, bam_path):
        """检测BAM文件是单端还是双端"""
        try:
    ...
    def align_and_coverm(self, sample):
        ...
        total_mapped = int(mapped_res.stdout.strip() or 1)
        logger.info(f"  ├─ 总reads: {global_reads:,} | 比对上: {total_mapped:,} ({total_mapped/global_reads*100:.2f}%%)")

        # ==================== 检测单双端 ====================
        is_paired = self.detect_paired_mode(raw_bam)
        pair_mode = "双端(PE)" if is_paired else "单端(SE)"
        logger.info(f"  └─ 检测模式: {pair_mode}")

        # ==================== CoverM 过滤阶段 ====================
        logger.info(f"🔨 [{sname}] 开始质量过滤 ({pair_mode})...")
        filter_cmd = [
        ...
        # 根据单双端自动添加 --proper-pairs-only
        if is_paired:
            filter_cmd.append('--proper-pairs-only')
            logger.info(f"  ├─ 启用选项: --proper-pairs-only (双端配对验证)")
        ...
        # 统计过滤后的reads
        filtered_res = subprocess.run(['samtools', 'view', '-c', str(filt_bam) + '.unsorted'], capture_output=True, text=True)
        filtered_reads = int(filtered_res.stdout.strip() or 0)
        logger.info(f"  ├─ 过滤参���: len≥{self.args.min_aln_len}bp, id≥{self.args.min_pid*100:.0f}%%, prop≥{self.args.min_aln_prop*100:.0f}%%")
        logger.info(f"  └─ 过滤后reads: {filtered_reads:,} ({filtered_reads/total_mapped*100:.2f}% of mapped)")
    ...
    def run_em_taxid_allocation(self, filt_bam_path, sname):
        """EM算法分配read到taxid，增加进度输出"""
        logger.info(f"🔄 [{sname}] 开始EM算法分配reads...")
        ...
        with pysam.AlignmentFile(filt_bam_path, "rb") as bam:
            # 第一遍：确定每条read的最优比对
            total_in_bam = bam.count()
            with tqdm(total=total_in_bam, desc=f"  ├─ EM扫描1: 找最优比对", unit="reads", leave=False) as pbar:
        ...
        logger.info(f"  └─ EM扫描1完成: {total_reads_processed:,} reads处理")
        ...
        logger.info(f"  ├─ EM扫描2: 计算分配权重...")
        em_counts, uniq_counts, multi_counts = defaultdict(float), defaultdict(int), defaultdict(int)
        
        with tqdm(total=len(read_best_data), desc=f"  ├─ EM扫描2: 权重分配", unit="reads", leave=False) as pbar:
            for data in read_best_data.values():
        ...
        unique_mapped = len([d for d in read_best_data.values() if len(d['taxids']) == 1])
        multi_mapped = len([d for d in read_best_data.values() if len(d['taxids']) > 1])
        logger.info(f"  └─ EM完成: 独特比对{unique_mapped:,}条, 多重比对{multi_mapped:,}条")
    ...
    def process_sample(self, sample):
        ...
        try:
                    ...
                    with gzip.open(stat_file, 'rt') as f:
                        for line in f:
                            if not line.startswith('#'):
                                p = line.split()
                                if len(p) >= 6: 
                                    depth_dict[p[0]] = {'Cov': float(p[4]), 'Dep': float(p[5])}
                                    virus_count += 1
                    logger.info(f"  └─ Pandepth完成: {virus_count} 个参考序列深度计算")
                except Exception as e:
            ...
            logger.info(f"✅ [{sname}] 完成: 检出 {len(results)} 个病毒")
            return results
        except Exception as e:
    ...
    def extract_filtered_reads(self, final_df_filtered):
        """【增强版】从 BAM 中提取过滤后病毒reads
        自动检测单双端，分别输出匹配格式的 FASTQ.GZ
        """
        if not self.args.extract or len(final_df_filtered) == 0:
            return

        logger.info(f"📦 正在提取 {len(final_df_filtered)} 个过滤后病毒的 reads（单双端自适应）...")
        
        sample_virus_map = defaultdict(set)
        for row in final_df_filtered.iter_rows(named=True):
            sample_virus_map[row['Sample']].add(row['Rep_Accession'])

        for sample in self.samples:
            sname = sample['name']
            if sname not in sample_virus_map:
                continue
                
            virus_set = sample_virus_map[sname]
            sorted_bam = self.output_dir / 'bam' / f'{sname}.sorted.bam'
            
            if not sorted_bam.exists():
                logger.warning(f"⚠️ BAM 文件不存在: {sorted_bam}")
                continue

            # 检测是单端还是双端
            is_paired = self.detect_paired_mode(sorted_bam)

            # 收集所有过滤后的 reads
            reads_by_qname = defaultdict(list)
            try:
                bamfile = pysam.AlignmentFile(str(sorted_bam), "rb")
                for read in bamfile.fetch(until_eof=True):
                    if read.is_unmapped:
                        continue
                ...
                logger.error(f"❌ 提取 reads 失败 ({sname}): {e}")
                continue

            # 根据单双端分别输出
            if is_paired:
                # ========== 双端模式 ==========
                fq1_path = self.output_dir / 'extract' / f"{sname}.filtered_viral_reads_1.fastq.gz"
                fq2_path = self.output_dir / 'extract' / f"{sname}.filtered_viral_reads_2.fastq.gz"
                
                count1 = count2 = 0
                with gzip.open(fq1_path, 'wt') as fq1, gzip.open(fq2_path, 'wt') as fq2:
                    for qname, reads in reads_by_qname.items():
                        r1 = r2 = None
                        ...
                        if r1:
                            query_seq = r1.query_sequence
                            query_qual = ''.join(chr(q + 33) for q in r1.query_qualities)
                            fq1.write(f"@{r1.query_name}\n{query_seq}\n+\n{query_qual}\n")
                            count1 += 1
                        
                        if r2:
                            query_seq = r2.query_sequence
                            query_qual = ''.join(chr(q + 33) for q in r2.query_qualities)
                            fq2.write(f"@{r2.query_name}\n{query_seq}\n+\n{query_qual}\n")
                            count2 += 1
                
                logger.info(f"✅ 提取 {sname}：双端模式 - R1: {count1:,} 条，R2: {count2:,} 条")
                ...
                count1 = 0
                with gzip.open(fq1_path, 'wt') as fq1:
                    for reads in reads_by_qname.values():
                        for r in reads:
                            query_seq = r.query_sequence
                            query_qual = ''.join(chr(q + 33) for q in r.query_qualities)
                            fq1.write(f"@{r.query_name}\n{query_seq}\n+\n{query_qual}\n")
                            count1 += 1
                
                logger.info(f"✅ 提取 {sname}：单端模式 - {count1:,} 条 reads")
    ...
    def summarize_results_polars(self, all_metrics):
        ...
        if len(df_white) > 0:
            ...
            # 提取过滤后的 reads
            if self.args.extract:
                self.extract_filtered_reads(df_white)
...
【Reads 提取参数 - Reads Extraction】
  --extract           启用 reads 提取模式，自动识别单双端输出格式
                      单端输出：{sample}.filtered_viral_reads_1.fastq.gz
                      双端输出：{sample}.filtered_viral_reads_1.fastq.gz + {sample}.filtered_viral_reads_2.fastq.gz
                      输出位置：output_dir/extract

【关键修复说明】
  ✅ 比对过程实时输出统计 (总reads, 比对上, 比对率)
  ✅ CoverM过滤实时输出统计 (过滤参数, 过滤后reads数)
  ✅ 双端数据自动启用 --proper-pairs-only 配对验证
  ✅ Pandepth过程实时输出完成统计
  ✅ EM算法分配过程分二步展示(扫描1: 找最优比对, 扫描2: 权重分配)
  ✅ 过滤阈值基于代表序列的真实数据
  ✅ --extract 自动识别单双端，输出匹配格式的 FASTQ.GZ

【使用示例】

  # 标准模式（推荐） - 双端数据自动启用proper-pairs
  python batch_virus_depth32.py -i input/ -r ref.fa --ref_info info.tsv \\
    --coverage 50 --meandepth 10

  # 启用 reads 提取（自动识别单双端）
  python batch_virus_depth32.py -i input/ -r ref.fa --ref_info info.tsv \\
    --coverage 50 --meandepth 10 --extract

  # 详细日志输出
  python batch_virus_depth32.py -i input/ -r ref.fa --ref_info info.tsv \\
    --coverage 30 --meandepth 5 --sp_thresh 93.0 --extract --verbose
        """
    )
    ...
    parser.add_argument('--extract', action='store_true', 
                        help='启用 reads 提取模式，自动识别单双端并输出 .fastq.gz 格式')
