...
class VirusAnalysisPipeline:
    ...
    def extract_fastas_with_python(self, target_ids, virus_to_tax, out_dir):
        ...
        def save_record(vid, seq_list):
            if vid in target_set:
                ...
                if len(safe_tax) > 50: safe_tax = safe_tax[:50] # 防止路径过长
                
                # 最终文件夹格式：Taxonomy_Accession
    ...
    def _run_single_consensus(self, task):
        ...
        try:
            ...
            log_block.append(f"✅ {sample} 构建成功! 存入目录: {folder_name}/")
...
def main():
    parser = argparse.ArgumentParser(
        description='病毒分析终极版 (带断点续跑、最优毒株筛选、带分类学归档的共识提取)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
增强功能说明:
  --resume   : 开启断点续跑。将自动跳过已完成的比对、深度计算、共识提取，以及自动加载 NCBI 爬取缓存。
  --consensus: 基于去重最优代表序列生成共识，将建立如 Taxonomy_Accession/ 这样的分类学可读目录。
        """
    )
