...
class VirusAnalysisPipeline:
    ...
    def extract_fastas_with_python(self, target_ids, out_dir):
        """Python 内置极速 FASTA 提取器，取代缓慢的 Seqkit 多次调用"""
        logger.info("🧬 正在通过 Python 提取参考 FASTA 序列...")
        ...
        def save_record(vid, seq_list):
            if vid in target_set:
                with open(out_dir / f"{vid}.ref.fasta", 'w') as outf:
                    outf.write(f">{vid}\n" + "".join(seq_list) + "\n")
                found.add(vid)
    ...
    def _run_single_consensus(self, task):
        ...
        ref_fasta = consensus_dir / f"{virus}.ref.fasta"
        fixed_bam = consensus_dir / f"{sample}.{virus}.fixed.bam"
        out_fasta = consensus_dir / f"{sample}.{virus}.consensus.fasta"
        
        if not ref_fasta.exists() or not bam_file.exists():
            return False
            
        # 根据需求：使用向下取整的 MeanDepth 作为最低覆盖阈值
        min_depth = int(math.floor(mean_depth))
        
        try:
            # 1. 过滤冗余 @SQ 头部文件并提取病毒对应的比对信息
            # 等价于 bash 中的: samtools view -h bam | awk '/^@SQ/ && $2 != "SN:$Virus" {next} {print}' | samtools view -b
    ...
    def generate_report(self, df_all):
        ...
        with open(report_file, 'w') as f:
            ...
            f.write(f"生成共识序列: {'是 (保存在 consensus 目录)' if self.args.consensus else '否'}\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
增强功能说明:
  --consensus: 开启共识序列构建。它会解析过滤后的合格病毒记录，使用 awk 安全过滤 BAM 文件头，
               并调用 viral_consensus 自动生成所有符合条件病毒的 consensus.fasta 序列。

使用示例:
  # 基础分析 + RVDB 注释 + 自动生成共有序列
  python batch_virus_depth.py --tool bwa-mem2 --input_dir fastq --reference virus.fasta \\
    ...
    align_group.add_argument('--consensus', action='store_true', help='对发现的病毒结果自动生成共识序列')
