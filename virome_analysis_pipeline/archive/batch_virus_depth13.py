...
4. 包含智能深度截断与带 Taxonomy 标识整理的共识序列构建
5. 包含阳性reads提取、变异检测及断点续跑(--resume)、全自动 R 绘图出图
"""
...
class VirusAnalysisPipeline:
    ...
    def fetch_ncbi_taxonomy(self, accessions):
        ...
        logger.info(f"🌐 正在通过 NCBI 联网获取 {len(to_fetch)} 个本地未注释病毒的分类信息...")
    ...
    def save_results(self, all_results):
        with Timer("汇总去重与保存结果"):
            ...
            # --- 改变1: 未过滤的所有结果现在保存为 all_viruses.raw.{format} ---
            summary_file = summary_dir / f'all_viruses.raw.{self.args.format}'
    ...
    def extract_fastas_with_python(self, target_ids, virus_to_tax, out_dir):
        """内置极速 FASTA 提取器，并创建带有 Taxonomy 的专属目录"""
    ...
    def _run_single_read_extraction(self, sample):
        """根据包含病毒比对的样本，单独提取其 reads 并输出单双端"""
        ...
        try:
            # -F 4: 排除所有未比对上的Reads，仅提病毒阳性Reads
            # 注意: samtools fastq -1 / -2 输出配对数据；若不加-s或-0，所有孤立单端/未配对数据会默认输出到stdout
            # 将stdout用管道连接pigz，完美解决纯单端数据(-0)和配对里的orphan reads(-s)的安全写入
            cmd = (
                f"samtools fastq --threads 16 -F 4 "
                f"-1 >(pigz -c > '{r1}') "
                f"-2 >(pigz -c > '{r2}') "
                f"'{bam_file}' | pigz -c > '{rs}'"
            )
            # 由于使用了 process substitution `>()`，需要通过 /bin/bash 执行
            subprocess.run(cmd, shell=True, check=True, executable='/bin/bash', stderr=subprocess.PIPE)
            
            # 智能清理: 如果该样本全是单端(没有R1/R2) 或 全是双端(没有单端reads)，对应生成的 gz 会是 20 字节空头文件，应予以删除
            for f in[r1, r2, rs]:
    ...
    def extract_mapped_reads(self, best_results):
        ...
        # 只针对出现在最优代表株列表(阳性检测出病毒)里的 Sample 进行提取
    ...
    def generate_report(self, df_best, total_raw_records):
        ...
        with open(report_file, 'w') as f:
            ...
            f.write(f"初筛包含冗余记录总数: {total_raw_records} (保存在 all_viruses.raw.{self.args.format})\n")
            f.write(f"同源去重后核心记录数: {len(df_best)} (保存在 all_viruses.best.summary.{self.args.format})\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description='病毒分析终极版 (带断点续跑、最优毒株筛选、Reads 提取、共识及突变检测)',
