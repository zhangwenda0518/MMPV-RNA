...
5. 包含变异检测 (freebayes/ivar/lofreq) 及断点续跑(--resume)、全自动 R 绘图出图
"""
...
class VirusAnalysisPipeline:
    ...
    def run_pipeline(self):
        ...
        if all_results:
            ...
            if self.args.call_variants and best_records:
...
def main():
    parser = argparse.ArgumentParser(
        description='病毒分析终极版 (带断点续跑、最优毒株筛选、带分类学归档的共识提取和变异检测)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
增强功能说明:
  --resume       : 开启断点续跑。将自动跳过已完成的比对、深度计算、共识提取和变异检测。
