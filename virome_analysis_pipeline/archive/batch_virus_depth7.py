...
class VirusAnalysisPipeline:
            ...
            if 'Virus' in df_all.columns:
                ...
                if unannotated_viruses:
                    if self.args.email:
                        ...
                        logger.warning(f"💡 发现 {len(unannotated_viruses)} 个病毒未能解析分类。如需通过 NCBI 自动获取，请配置 --email 参数。")
    ...
    def generate_report(self, df_all):
        ...
        with open(report_file, 'w') as f:
            ...
            f.write(f"共识序列: {'已生成 (保存在 consensus 文件夹下)' if self.args.consensus else '否'}\n\n")
...
增强功能说明:
  ...
  联网解析:
    如果输入了 --email，程序会在本地 RVDB 解析完毕后，将仍标记为 Unannotated 
    的序列通过 NCBI Entrez 接口在线查询。
        """
    )
