...
2. 包含 RVDB 极速流式解析
3. 包含智能深度截断与按子目录整理的共识序列构建
4. 包含独立的单样本全量日志 (存放软件执行与报错细节) 和纯净的终端进度条 UI
"""
...
class VirusAnalysisPipeline:
    ...
    def fast_load_rvdb_taxonomy(self, found_viruses):
        ...
        logger.info(f"📂 正在快速流式扫描 RVDB 注释文件...")
            ...
            with open(self.args.rvdb_taxon, 'r', encoding='utf-8') as f:
                ...
                for line in f:
                    ...
                    if acc in target_exact or acc_base in target_base or acc in target_base:
                        ...
                        found_count += 1
            logger.info(f"✅ 扫描完成，提取到 {found_count} 条分类信息")
            return taxon_dict
    ...
    def run_pipeline(self):
        ...
        if self.args.parallel and self.args.threads > 1:
            ...
            with ProcessPoolExecutor(max_workers=max_parallel_jobs) as executor:
                ...
                with tqdm(total=len(self.samples), desc="进度", unit="样本", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {postfix}, {rate_fmt}]') as pbar:
                    for future in as_completed(future_to_sample):
                        sample = future_to_sample[future]
                        # 动态更新进度条后缀，实现纯净且直观的 UI
                        pbar.set_postfix_str(f"处理样本: {sample['name']}中")
                        res = future.result()
                        if res is not None: all_results.extend(res)
                        pbar.update(1)
        else:
            with tqdm(self.samples, desc="进度", unit="样本", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {postfix}, {rate_fmt}]') as pbar:
                for sample in pbar:
                    pbar.set_postfix_str(f"处理样本: {sample['name']}中")
    ...
    def extract_fastas_with_python(self, target_ids, out_dir):
        ...
        logger.info("🧬 正在提取参考 FASTA 并建立病毒分子目录...")
    ...
    def _run_single_consensus(self, task):
        ...
        fixed_bam = virus_dir / f"{vname}.{sample}.fixed.bam"
        out_fasta = virus_dir / f"{vname}.{sample}.consensus.fasta"
        ...
        log_block.append(f"原始 MeanDepth: {mean_depth} -> 动态设置 min_depth: {min_depth}")
    ...
    def build_consensus_sequences(self, all_results):
        ...
        with Timer("共识序列构建"):
            ...
            tasks = [(r['Sample'], r['Virus'], r['MeanDepth']) for r in all_results]
            success_count = 0
            ...
            with ProcessPoolExecutor(max_workers=min(len(tasks), self.args.threads)) as executor:
                ...
                with tqdm(total=len(tasks), desc="构建进度", unit="文件", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {postfix}, {rate_fmt}]') as pbar:
                    for future in as_completed(futures):
                        task = futures[future]
                        pbar.set_postfix_str(f"共识提取: {task[0]} ({task[1][:10]}..)中")
                        if future.result(): success_count += 1
                        pbar.update(1)
            
            logger.info(f"✅ 成功生成了 {success_count} 个按病毒分类归档的共识序列文件")
    ...
    def generate_report(self, df_all):
        ...
        with open(report_file, 'w') as f:
            ...
            if self.args.rvdb_taxon: f.write(f"使用RVDB注释文件: {self.args.rvdb_taxon}\n")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
增强功能说明:
  --consensus: 开启共识序列构建。按病毒名在 consensus 下建立独立目录。
               动态截断 --min_depth (最高10，否则向下取整)。
  日志体系:
    极度纯净的控制台输出 UI + 独立样本的底层完整日志追踪 (存放于 logs/ 下)。
        """
    )
    ...
    align_group.add_argument('--consensus', action='store_true', help='对发现的病毒结果自动生成归档的共识序列')
