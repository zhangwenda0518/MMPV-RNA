...
def main():
    ...
    parser.add_argument("--resume", action="store_true", help="断点续传: 跳过已有组装输出的任务")
    ...
    for _, row in df.iterrows():
        ...
        tax_raw = row['Taxonomy'] if 'Taxonomy' in df.columns else 'Unknown_Virus'
        ...
        ext_reads = find_extracted_reads(sample, virus, str(downstream / "reads"))
        if not ext_reads:
            continue
        ...
        if args.resume:
            ...
            for probe in ["final.fasta", "scaffolds.fasta", "contigs.fasta"]:
                if (task_outdir / probe).is_file() and (task_outdir / probe).stat().st_size > 100:
                    done = True
                    break
            if done:
                skipped_resume += 1
    ...
    if skipped_resume:
        print(f"⏭  断点续传: 跳过了 {skipped_resume} 个已完成的任务。")
    ...
    with ProcessPoolExecutor(max_workers=args.jobs) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(tasks), desc="全长打磨中"):
            res = future.result()
            if "✅" in res:
                success_count += 1
            tqdm.write(res)
