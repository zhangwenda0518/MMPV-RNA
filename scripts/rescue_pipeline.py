...
def branch_c(fail_fa, fastq_dir, work_dir,
    ...
    def _do(rec):
        ...
        if blast_db:
            run_blastn(tmp, blast_db, sub_dir / "blastn.tsv", threads_per)

        qs = run_checkv(tmp, sub_dir / "cv", checkv_db, threads_per)
...
def main():
    ...
    if args.resume and centroids_final.is_file() and centroids_final.stat().st_size > 0:
        ...
        print(f"  A:{cnt_a}  C:{cnt_b}  D:{cnt_c}  →  {total_m} 条  →  {merged}")
