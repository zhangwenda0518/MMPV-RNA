...
<section class="stage {border_cls}" id="stage-{short}">
  ...
  {chart_html}
    ...
    # ── 数据附录: 嵌入所有 TSV 表预览 ──
    data_appendix = ""
    tsv_files = sorted([f for f in report_dir.glob("*.tsv") if f.name != "stage_summary.tsv"])
    if tsv_files:
        rows_html_app = ""
        for tf in tsv_files:
            rows = _read_tsv(tf)
            ...
            th_html = "".join(f"<th>{_esc(c)}</th>" for c in cols)
            tr_html = ""
            for r in preview:
...
<h3>Data Appendix — Summary Tables</h3>
<div style="padding:12px 22px">{rows_html_app}</div>
</div>'''
...
.app-detail{{border-bottom:1px solid var(--border);padding:8px 0}}
.app-detail summary{{cursor:pointer;font-size:13px;padding:4px 0;color:var(--text)}}
.app-detail summary:hover{{color:var(--blue)}}
