...
5. 包含断点续跑(--resume)、单样本全量日志追踪及任务结束后的绘图指引
"""
...
class VirusAnalysisPipeline:
    ...
    def _run_single_consensus(self, task):
        ...
        safe_tax = re.sub(r'[^\w\-.]', '_', taxonomy)
        vname = re.sub(r'[\\/*?:"<>|]', "_", virus)
        virus_dir = consensus_dir / f"{safe_tax}.{vname}"
    ...
    def build_consensus_sequences(self, best_results):
        ...
        with Timer("核心共识序列构建"):
            ...
            tasks = [(r['Sample'], r['Virus'],r['taxonomy'], r['MeanDepth']) for r in best_results]
            success_count = 0
    ...
    def print_plotting_instructions(self, best_summary_file):
        ...
        logger.info("为了方便您定制化调整图像，可视化环节已被分离为独立的 R 脚本。")
        logger.info(f"最优核心结果文件位置：{best_summary_file}")
        logger.info("\n请在命令行中直接运行以下命令生成多维度分析图：")
        logger.info(f"\033[1;36mRscript ~/bin/virus_frequency_plot.R -i {best_summary_file} -o {plots_out_prefix} --multi-plot --log10-transform\033[0m")
        logger.info("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(
        description='病毒分析终极版 (带断点续跑、最优毒株筛选、共识自动提取与独立绘图提示)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
增强功能说明:
  --resume   : 开启断点续跑。将自动跳过已完成比对、深度计算和共识提取的文件。
  --consensus: 基于去重后的同源最优代表序列自动生成共识FASTA。
        """
    )
