...
class VirusAnalysisPipeline:
    ...
    def fetch_ncbi_taxonomy(self, accessions):
        """利用 NCBI Entrez 接口在线补全未知病毒分类"""
        try:
            from Bio import Entrez, SeqIO
        except ImportError:
            logger.warning("⚠️ 找不到 biopython 模块，请运行 'pip install biopython' 安装。跳过 NCBI 在线解析。")
            return {}
        ...
        for acc in tqdm(accessions, desc="NCBI 解析进度", unit="seq"):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    taxonomy_name = record.annotations.get('organism', '-')
            ...
            if not success:
                logger.debug(f"[错误] 获取 {acc} 时发生错误: {last_error}")
                ncbi_dict[acc] = {
    ...
    def align_sample(self, sample):
        ...
        if sorted_bam.exists() and self.args.skip_alignment:
            self.write_sample_log(sample_name, f"⏭️  跳过比对，直接使用已有BAM: {sorted_bam}")
            return sorted_bam
        ...
        self.write_sample_log(sample_name, f"Index CMD: {' '.join(index_cmd)}")
        res = subprocess.run(index_cmd, capture_output=True, text=True)
        if res.returncode != 0:
            self.write_sample_log(sample_name, f"❌ 构建索引失败:\n{res.stderr}")
            return None
    ...
    def get_idxstats(self, bam_file, sample_name):
        ...
        self.write_sample_log(sample_name, f"CMD: {' '.join(cmd)}")
        result = subprocess.run(cmd, capture_output=True, text=True)
        if result.returncode != 0:
    ...
    def run_pandepth(self, bam_file, sample_name):
        ...
        if stat_file.exists() and self.args.skip_depth:
            self.write_sample_log(sample_name, f"⏭️  跳过已存在的深度文件: {stat_file}")
            return stat_file
            
        self.write_sample_log(sample_name, "\n--- 3. 深度计算 (pandepth) ---")
        pandepth_cmd = ['pandepth', '-a', '-i', str(bam_file), '-o', str(output_prefix), '-t', str(min(10, self.args.threads))]
        self.write_sample_log(sample_name, f"CMD: {' '.join(pandepth_cmd)}")
    ...
    def parse_stat_file(self, stat_file, sample_name, idx_data):
        ...
        try:
            ...
            self.write_sample_log(sample_name, f"\n--- 4. 数据解析与定量 ---")
            self.write_sample_log(sample_name, f"✅ 解析完毕, 筛选出 {len(results)} 条符合阈值的病毒记录。")
            return results
        except Exception as e:
    ...
    def process_sample(self, sample):
        ...
        self.write_sample_log(sample_name, f"[{start_time.strftime('%Y-%m-%d %H:%M:%S')}] 开始处理测序样本: {sample_name}", mode='w')
        ...
        try:
            ...
            self.write_sample_log(sample_name, f"\n[{end_time.strftime('%Y-%m-%d %H:%M:%S')}] 🎉 样本整体处理流程结束 [耗时: {Timer.format_duration(duration.total_seconds())}]")
            return res
        except Exception as e:
            self.write_sample_log(sample_name, f"\n❌ 样本执行期间发生致命错误: {str(e)}")
            return None
    ...
    def run_pipeline(self):
        ...
        if all_results:
            # save_results 将返回去重筛选后的最优结果集 best_results
            best_results = self.save_results(all_results)
            if self.args.consensus and best_results:
                self.build_consensus_sequences(best_results)
        else:
            logger.warning("⚠️ 未找到任何满足深度和覆盖度过滤条件的病毒记录")
    ...
    def save_results(self, all_results):
        """整合分类信息，输出全量结果，并提取最优代表株(Best Representative)"""
        with Timer("汇总去重与保存结果"):
            ...
            if 'Virus' in df_all.columns:
                ...
                if unannotated_viruses:
                    if self.args.email:
                        ...
                        logger.warning(f"💡 发现 {len(unannotated_viruses)} 个病毒未能解析分类，如需 NCBI 自动获取请配置 --email 参数。")
            ...
            # --- 3. 【核心优化】：同源去除，提取最优核心序列 (Best Selection) ---
            if 'taxonomy' in df_all.columns:
                # 针对未能解析分类的病毒，以它原本的 Virus(Accession) 作为分组主键，防止强行被合并成一项
            ...
            logger.info(f"✅ 【去重优化】最优代表株结果已保存至 all_viruses.best.summary ({len(df_best)} 条核心记录)")
            
            # 以后的子报表直接基于去重优化后的 df_best 生成
            if 'Virus' in df_best.columns:
                for virus in df_best['Virus'].unique():
                    df_virus = df_best[df_best['Virus'] == virus].copy()
                    vname = re.sub(r'[\\/*?:"<>|]', "_", virus)
                    vfile = summary_dir / f'{vname}.best.summary.{self.args.format}'
            ...
            # 返回精简后的结果，供下游共识序列(consensus)模块专用
            return df_best.to_dict('records')

    # ================= 共识序列处理核心模块 =================
    
    def extract_fastas_with_python(self, target_ids, out_dir):
        """Python 内置极速 FASTA 提取器，并为每个病毒建立独立文件夹"""
        logger.info("🧬 正在提取核心参考 FASTA 并建立病毒分子目录...")
    ...
    def build_consensus_sequences(self, best_results):
        ...
        with Timer("核心共识序列构建"):
            # 此时的 best_results 已经是剔除了低覆盖度冗余株后的纯净核心数组
            ...
            logger.info(f"✅ 成功提取了 {success_count} 个按病毒分类归档的最优共识序列文件")
    ...
    def generate_report(self, df_best, total_raw_records):
        ...
        logger.info(f"📄 终版分析报告已保存: {report_file}")
    
def main():
    parser = argparse.ArgumentParser(
        description='病毒序列比对、深度统计、多维度定量分析及共识序列生成核心流程',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
增强功能说明:
  --consensus: 开启共识序列构建。它会基于同源去重后保留下来的最优覆盖代表序列建立独立目录。
               动态截断 --min_depth (最高10，否则向下取整)。
  去重机制:
               程序会自动合并同一样本下命中多个亚型的 Taxonomy，保留 Coverage(%) 与深度最强的结果。
               原始冗余记录保存在 all_viruses.summary.tsv。
               筛选后的分析基准保存在 all_viruses.best.summary.tsv。
        """
    )
