...
6. [终极优化] 源头剔除 256/2048 次优与嵌合比对，确保 Depth 与 Mapped 严格基于真实物理序列
7. [终极优化] 完美兼容 5 大软件的 Unique_Reads / Multi_Reads 极速分离算法 (基于 MAPQ 标尺)
8. [新增] 样本级全局 BAM 测序比对质量评估 (基于 samtools stats)
"""
...
class VirusAnalysisPipeline:
    ...
    def align_sample(self, sample):
        ...
        # [核心优化]: 三级管道处理，落盘前剔除次优(256)和嵌合(2048)序列
        # 确保下游所有的 Depth 与 Mapped 计算只基于真实的物理主比对序列！
        ...
        self.write_sample_log(sample_name, f"Filter & Sort CMD: {' '.join(view_cmd)} | {' '.join(sort_cmd)}")
        ...
        if align_proc.returncode != 0 and align_proc.returncode is not None:
        ...
        self.write_sample_log(sample_name, f"✅ 比对与深度净化(去除次优/嵌合)成功。\n[Stderr 截取输出]:\n{align_stderr[:1000]}")
    ...
    def get_unique_multi_stats(self, bam_file, sample_name):
        ...
        # AWK 逻辑终极优化 (兼容所有比对软件)：
        # 因为在 align_sample 中已经抛弃了 256 和 2048，此处只过滤 4(未必对) 也是可以的
        # 但保留 -F 2308 是最稳健的写法(防止用户提供未过滤的外部 BAM 进行断点续跑)
        awk_script = """
        {
            ref = $3;
            if (ref == "*") next; 
    ...
    def save_results(self, all_results):
        with Timer("汇总去重与保存结果"):
            ...
            logger.info(f"✅ 全量 Mapped 记录已保存至 all_viruses.raw ({total_raw_records} 条)")
            logger.info(f"✅ 阈值过滤记录已保存至 all_viruses.summary ({total_filtered_records} 条)")
