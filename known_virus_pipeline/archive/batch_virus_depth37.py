#!/usr/bin/env python3
"""
宏病毒鉴定与精确定量管线 (Virus Quantification - V34.2 Production Gold Master)
================================================================================
【V34.2 工业级防崩与性能榨取】
1. [格式兼容] 全面支持 FASTA/FASTQ (.fa, .fasta, .fq, .fastq 及 .gz 压缩格式)。
2. [智能寻址] 彻底重写单双端混合识别逻辑，完美支持同一目录下单/双端混合、孤立 R2 等复杂情况。
3. [数学安全] 彻底封堵 TPM/FPKM 计算中的 ZeroDivision 漏洞。
4. [内存熔断] 废弃 map 一波流，全面引入 as_completed 异步收割，解决百 GB 级并发 OOM。
5. [严谨降级] 设立 0.1% 真实突变下限，严格隔离测算失败(Failed)与真实新种(Novel)。
6. [极速提取] 新增 --extract，根据源格式智能调用 samtools fasta/fastq 和 pigz 提取阳性序列。
================================================================================
"""
...
class VirusQuantificationPipeline:
    ...
    def find_samples(self):
            ...
            for sname, reads in file_dict.items():
                if self.args.single_end:
                    ...
                    if 'R1' in reads and 'R2' in reads:
                        samples.append({'name': sname, 'r1': reads['R1'], 'r2': reads['R2']})
                    elif 'R1' in reads:
                        samples.append({'name': sname, 'r1': reads['R1'], 'r2': None})
                    elif 'R2' in reads:
                        # 极端情况：SRA 只下载下来了 _2.fa.gz (孤立 R2)
                        samples.append({'name': sname, 'r1': reads['R2'], 'r2': None})
                    elif 'SE' in reads:
                        samples.append({'name': sname, 'r1': reads['SE'], 'r2': None})

        # 解析用户手动提供的 sample_list
        ...
        se_count = len(self.samples) - pe_count
        logger.info(f"✅ 成功寻址 {len(self.samples)} 个测序样本 (自动识别出: {pe_count} 个双端, {se_count} 个单端)")
    ...
    def align_and_coverm(self, sample):
        ...
        if sample['r2']:
            ...
            self.write_sample_log(sname, "-> 探测到双端数据，已自动开启 --proper-pairs-only", level="info")
    ...
    def _compute_ani_pi_worker(self, task):
        ...
        try:
            ...
            self.write_sample_log(sname, f"ANI 计算异常: {e}", level="warning")
            return {'Sample': sname, 'Rep_Accession': ref, 'Avg_Read_ANI': None, 'Avg_Pi': None}
    ...
    def _run_single_read_extraction(self, sample):
        ...
        for s in self.samples:
            if s['name'] == sample:
                is_fasta_input = any(s['r1'].lower().endswith(ext) for ext in ['.fa', '.fasta', '.fa.gz', '.fasta.gz'])
                break
                
        ext_name = "fasta.gz" if is_fasta_input else "fastq.gz"
        r1 = reads_dir / f"{sample}_virus_1.{ext_name}"
        r2 = reads_dir / f"{sample}_virus_2.{ext_name}"
        rs = reads_dir / f"{sample}_virus_single.{ext_name}"
        
        if self.args.resume and (r1.exists() or r2.exists() or rs.exists()):
            self.write_sample_log(sample, "⏭️ [断点续跑] 跳过提取，目标文件已存在", level="info")
            return True
            
        if not bam_file.exists(): return False

        try:
            samtools_subcmd = "fasta" if is_fasta_input else "fastq"
            cmd = (
                f"samtools {samtools_subcmd} -F 4 "
                f"-1 >(pigz -p 2 -c > '{r1}') "
                f"-2 >(pigz -p 2 -c > '{r2}') "
            ...
            self.write_sample_log(sample, "✅ 病毒序列提取成功! 存入目录: reads/", level="info")
            return True
        except subprocess.CalledProcessError as e:
            err_msg = e.stderr.decode() if isinstance(e.stderr, bytes) else (e.stderr or str(e))
            self.write_sample_log(sample, f"❌ 序列提取失败:\n{err_msg}", level="error")
            return False
    ...
    def extract_mapped_reads(self, best_df):
        ...
        with ProcessPoolExecutor(max_workers=min(len(unique_samples), self.args.threads)) as executor:
            ...
            with tqdm(total=len(unique_samples), desc="[提取阳性序列]", unit="样本", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {postfix}]') as pbar:
                for future in as_completed(futures):
                    sample = futures[future]
                    pbar.set_postfix_str(f"最新完成: {sample}")
                    if future.result(): success_count += 1
                    pbar.update(1)
        
        logger.info(f"✅ 成功提取了 {success_count} 个阳性样本的病毒序列数据")
    ...
    def run_pipeline(self):
        ...
        with Timer("整体管线运行"):
            ...
            logger.info("🚀 开始多进程处理各样本 (为保证进度条整洁，样本子步骤详情已静默写入 logs/ 目录)...")
            res_list = []
            with ProcessPoolExecutor(max_workers=self.args.threads) as executor:
                futures = {executor.submit(self.process_sample, s): s for s in self.samples}
                with tqdm(total=len(self.samples), desc="[样本定量处理]", unit="样本", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {postfix}]') as pbar:
                    for future in as_completed(futures):
...
def main():
    parser = argparse.ArgumentParser(
        description='宏病毒鉴定与精确定量管线 V34.2',
...
【性能优化与输出控制参数】
  ...
  --extract           自动提取命中阳性病毒的序列和参考 FASTA (自动识别并对应输出 fastq/fasta)
  ...
  --verbose           关闭静默模式，将每个样本的【比对/清洗/计算】步骤细节输出至终端
    ...
    parser.add_argument('--extract', action='store_true', help='提取比对到病毒的序列文件和参考序列')
