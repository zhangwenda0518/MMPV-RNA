...
2. [数学安全] 彻底封堵 TPM/FPKM 计算中的 ZeroDivision 漏洞。
3. [内存熔断] 废弃 map 一波流，全面引入 as_completed 异步收割，解决百 GB 级并发 OOM。
4. [严谨降级] 设立 0.1% 真实突变下限，严格隔离测算失败(Failed)与真实新种(Novel)。
5. [工具拓展] 全面支持 bowtie2, bwa, bwa-mem2, minimap2, strobealign, hisat2。
6. [智能质控] 自动识别单/双端数据并开启 CoverM --proper-pairs-only；输出全局 BAM 质控。
7. [极速提取] 新增 --extract，根据源格式智能调用 samtools fasta/fastq 和 pigz 提取阳性序列。
8. [视觉体验] 引入防撕裂的多进程安全 tqdm 实时进度条体系。
================================================================================
"""
    ...
    def find_samples(self):
        ...
        if input_dir and self.args.single_end:
            for ext in valid_exts:
                for seq_file in input_dir.glob(f'*{ext}'):
                    sname = seq_file.name.replace(ext, '').replace('.gz', '')
                    for suf in ['_1', '_R1', '.R1', "R1_10239", '.1', '_unmapped', '.unmapped', '_trimmed', '.trimmed']:
                        if suf in sname: sname = sname.split(suf)[0]
                    samples.append({'name': sname, 'r1': str(seq_file), 'r2': None})
                    
        elif input_dir:
            patterns = [('*_1.f*', '*_2.f*'), ('*_R1*.f*', '*_R2*.f*'), ('*.R1.*', '*.R2.*'), 
                        ('*.1.f*', '*.2.f*'), ('*_1_*.f*', '*_2_*.f*'), 
                        ('*_unmapped.R1.f*', '*_unmapped.R2.f*'), ('*unmapped.R1.f*', '*unmapped.R2.f*'), 
                        ('*_1_unmapped.*', '*_2_unmapped.*'), ('*.unmapped.R1_10239.*', '*.unmapped.R2_10239.*')]
            found_files = set()
            for p1, p2 in patterns:
                for r1_file in input_dir.glob(p1):
                    if not any(r1_file.name.lower().endswith(ext) for ext in valid_exts):
                        continue
            ...
            if not samples:
                all_seqs = []
                for ext in valid_exts: all_seqs.extend(input_dir.glob(f'*{ext}'))
                fg = defaultdict(list)
                for f in all_seqs:
