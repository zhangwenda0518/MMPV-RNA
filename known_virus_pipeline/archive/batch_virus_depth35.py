...
7. [极速提取] 新增 --extract，基于 pigz 和 samtools 进程替换极速提取阳性 Reads 与序列。
...
class VirusQuantificationPipeline:
    ...
    def write_sample_log(self, sample_name, message, level="info"):
        """将日志写入文件。仅在发生错误或开启 verbose 时推送到终端，保护进度条"""
    ...
    def extract_mapped_reads(self, best_df):
        ...
        with ProcessPoolExecutor(max_workers=min(len(unique_samples), self.args.threads)) as executor:
            ...
            with tqdm(total=len(unique_samples), desc="[提取阳性Reads]", unit="样本", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {postfix}]') as pbar:
...
【性能优化与输出控制参数】
  ...
  --verbose           关闭静默模式，将每个样本的【比对/清洗/计算】步骤细节输出至终端
    ...
    parser.add_argument('--verbose', action='store_true', help='强制将所有子进程详情打印到屏幕 (可能导致进度条错乱)')
