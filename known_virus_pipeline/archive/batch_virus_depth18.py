...
6. 完美兼容 5 大软件的 Unique_Reads / Multi_Reads 分离统计算法
7. 样本级全局 BAM 测序比对质量评估 (基于 samtools stats)
8. 完美兼容 .fa/.fasta 输入及单双端数据混合自动发现
9. 强制清理分类名中的单引号/括号等特殊字符，防止引起shell断点报错
10. 过滤前置：仅对通过阈值过滤的有效病毒进行分类学和联网注释，极大提升速度
11. [升级] 整合 bcftools 硬过滤，并引入基于 MeanDepth 的智能动态阈值质控模型 (Dynamic VCF Filtering)
"""
...
class VirusAnalysisPipeline:
    ...
    def check_tools(self):
        ...
        if getattr(self.args, 'consensus', False): 
        ...
        if getattr(self.args, 'call_variants', False):
            ...
            if self.args.variant_caller in ['freebayes', 'lofreq']:
                required_tools['bcftools'] = 'VCF过滤与统计工具'
            ...
            with open(self.args.rvdb_taxon, 'r', encoding='utf-8') as f:
                ...
                for line in f:
                    ...
                    if acc in target_exact or acc_base in target_base or acc in target_base:
                        info = {
                            'description': parts[idx_desc] or '', 
                            'taxonomy': parts[idx_tax] or '', 
                            'taxonomy_id': parts[idx_taxid] or ''
                        }
                        taxon_dict[acc] = info
    ...
    def fetch_ncbi_taxonomy(self, accessions):
        """利用 BioPython 联网从 NCBI 抓取未识别序列的分类注释"""
        try: 
        ...
        with open(cache_file, 'a', encoding='utf-8') as cf:
            ...
            for acc in tqdm(to_fetch, desc="NCBI 解析进度", unit="seq"):
                ...
                if not success:
                    ncbi_dict[acc] = {
                        'description': "Unannotated (NCBI Error)", 
    ...
    def setup_output_directory(self):
        ...
        if getattr(self.args, 'extract_reads', False): 
            subdirs.append('reads')
        if getattr(self.args, 'consensus', False): 
            subdirs.append('consensus')
        if getattr(self.args, 'call_variants', False): 
            subdirs.append('variants')
    ...
    def build_index(self):
        """根据选用的比对软件自动构建参考序列的底层索引"""
        tool = self.args.tool
        ref_path = Path(self.args.reference).resolve()
        
        # 某些现代软件支持 on-the-fly 索引，无需预先构建
        ...
        logger.info(f"🏗️ 检查并构建 {tool} 索引...")
        with Timer(f"构建 {tool} 索引"):
            if tool == 'bwa':
                if Path(f"{prefix}.bwt").exists(): 
                    return
                cmd = ['bwa', 'index', '-p', str(prefix), str(ref_path)]
                
            elif tool == 'bwa-mem2':
                if Path(f"{prefix}.bwt.2bit.64").exists() or Path(f"{prefix}.0123").exists(): 
                    return
                cmd = ['bwa-mem2', 'index', '-p', str(prefix), str(ref_path)]
                
            elif tool == 'bowtie2':
                if Path(f"{prefix}.1.bt2").exists() or Path(f"{prefix}.1.bt2l").exists(): 
                    return
                cmd = ['bowtie2-build', '--threads', str(self.args.threads), str(ref_path), str(prefix)]
                
            elif tool == 'hisat2':
                if Path(f"{prefix}.1.ht2").exists() or Path(f"{prefix}.1.ht2l").exists(): 
                    return
    ...
    def find_samples(self):
        """自动检索目录或读取清单文件获取配对样本信息"""
        input_dir = Path(self.args.input_dir) if getattr(self.args, 'input_dir', None) else None
        ...
        if input_dir:
            ...
            for f in all_files:
                ...
                # 跳过可能是由于 R1 衍生出的 R2 文件，避免其独立成单端样本
        ...
        # 解析用户自行提供的 sample_list (可选)
        if getattr(self.args, 'sample_list', None):
    ...
    def align_sample(self, sample):
        ...
        # 强制建立 BAI 索引供后续截取
        subprocess.run(['samtools', 'index', str(sorted_bam)], capture_output=True, text=True)
        return sorted_bam
    ...
    def get_idxstats(self, bam_file, sample_name):
        """解析 BAM 文件提取绝对文库大小，为 RPM 和 TPM 定量作准备"""
        ...
        return {
            'stats': stats, 
            'global_total_reads': total_reads, 
            'total_rpk': total_rpk
        }
    ...
    def run_pandepth(self, bam_file, sample_name):
        """调用超高速轮子 Pandepth 进行病毒各个位点的深度计算"""
    ...
    def parse_stat_file(self, stat_file, sample_name, idx_data):
        """融合 Pandepth 输出与全局环境，计算 FPKM、RPM、TPM 定量指标"""
        if not idx_data: 
            return []
        ...
        try:
            with gzip.open(stat_file, 'rt') as f:
                for line in f:
                    ...
                    # 彻底丢弃完全没有 reads 分配的无效记录，极大减轻内存和 IO 负担
                    if mapped_reads == 0: 
                        continue 
    ...
    def run_pipeline(self):
        ...
        if self.args.parallel and self.args.threads > 1:
            ...
            if getattr(self.args, 'parallel_jobs', None): 
        ...
        # 将结果送入后处理阶段（汇总输出、画图、提取 reads、共识序列等）
        if all_results:
    ...
    def save_results(self, all_results):
        """进行三级表格输出：全量 Raw表 -> 阈值过滤的 Summary表 -> 同源去重的 Best代表表"""
        with Timer("汇总去重与保存结果"):
            df_all = pd.DataFrame(all_results)
            summary_dir = self.output_dir / 'summary'
            
            # 【第 1 级输出】: 毫无过滤的 Raw 数据
            raw_summary_file = summary_dir / f'all_viruses.raw.{self.args.format}'
            if self.args.format == 'csv': 
                df_all.to_csv(raw_summary_file, index=False)
            else: 
                df_all.to_csv(raw_summary_file, sep='\t', index=False)
                ...
                if self.args.rvdb_taxon:
                    ...
                    if taxon_db:
                        def get_tax_info(vid, key):
                            if vid in taxon_db: 
                                return taxon_db[vid][key]
                            base_id = vid.split('.')[0]
                            if base_id in taxon_db: 
                                return taxon_db[base_id][key]
            ...
            if self.args.format == 'csv': 
                ...
                df_filtered.to_csv(filtered_summary_file, sep='\t', index=False)

            # --- 生成 Best 最优数据截断机制 ---
            if not df_filtered.empty and 'taxonomy' in df_filtered.columns:
                df_filtered['group_tax'] = df_filtered.apply(
                    lambda x: x['taxonomy'] if x['taxonomy'] not in ['Unannotated', '-'] else x['Virus'], 
                    axis=1
                )
            else:
            ...
            if not df_filtered.empty:
                # 排序规则：在同一个 Taxonomy 下，挑选 Coverage 最高，深度最大的那个作为最优代表株 (Best)
                df_sorted = df_filtered.sort_values(
                    by=['Sample', 'group_tax', 'Coverage(%)', 'MeanDepth'], 
            ...
            # 【第 3 级输出】: 最终核实的代表株群落 Best 表
            best_summary_file = summary_dir / f'all_viruses.best.summary.{self.args.format}'
            if self.args.format == 'csv': 
                df_best.to_csv(best_summary_file, index=False)
            else: 
                df_best.to_csv(best_summary_file, sep='\t', index=False)
    ...
    def extract_fastas_with_python(self, target_ids, virus_to_tax, out_dir):
        """从极大的多重基因组库中，针对检出的特异病原体抽提出独立的 FASTA"""
        ...
        with open(self.args.reference, 'r') as f:
            for line in f:
                ...
                if line.startswith('>'):
                    if current_id: 
                        save_record(current_id, current_seq)
    ...
    def get_vcf_stats(self, vcf_file):
        """调用 bcftools 提取 VCF 中的基本统计信息"""
        stats_dict = {'SNPs': 0, 'INDELs': 0, 'MNPs': 0, 'others': 0, 'sites': 0}
        if not os.path.exists(vcf_file) or os.path.getsize(vcf_file) == 0:
            return stats_dict
            
        try:
            cmd = f"bcftools plugin counts '{vcf_file}'"
            res = subprocess.run(cmd, shell=True, capture_output=True, text=True)
            for line in res.stdout.split('\n'):
                if 'Number of SNPs:' in line: 
                    stats_dict['SNPs'] = int(line.split(':')[1].strip())
                elif 'Number of INDELs:' in line: 
                    stats_dict['INDELs'] = int(line.split(':')[1].strip())
                elif 'Number of MNPs:' in line: 
                    stats_dict['MNPs'] = int(line.split(':')[1].strip())
                elif 'Number of others:' in line: 
                    stats_dict['others'] = int(line.split(':')[1].strip())
                elif 'Number of sites:' in line: 
                    stats_dict['sites'] = int(line.split(':')[1].strip())
            return stats_dict
        except Exception:
            return stats_dict

    def _run_single_variant_calling(self, task):
        # 解包传递进来的参数，包含关键的深度参数进行策略控制
        sample, virus, taxonomy, mean_depth = task
        ...
        clean_vcf = virus_dir / f"{sample}.{folder_name}.filtered.vcf"
        
        # 兼容断点续跑并返回已有的质控报表
        if self.args.resume and out_file.exists() and out_file.stat().st_size > 0:
            self.write_sample_log(sample, f"\n⏭️ [断点续跑] 跳过变异检测，目标文件已存在: {out_file.name}")
            if out_ext == 'vcf' and clean_vcf.exists():
                raw_stats = self.get_vcf_stats(out_file)
                clean_stats = self.get_vcf_stats(clean_vcf)
                return {
                    'Sample': sample, 'Virus': virus, 'Taxonomy': taxonomy,
                    'Raw_SNPs': raw_stats['SNPs'], 'Raw_INDELs': raw_stats['INDELs'], 'Raw_Sites': raw_stats['sites'],
                    'Filtered_SNPs': clean_stats['SNPs'], 'Filtered_INDELs': clean_stats['INDELs'], 'Filtered_Sites': clean_stats['sites']
                }
            return True 
        ...
        try:
            ...
            if caller == 'freebayes':
                ...
                gff_param = f"-g '{self.args.ref_gff}'" if getattr(self.args, 'ref_gff', None) else ""
            ...
            # ================= [核心 VCF 硬过滤与智能动态阈值模块] =================
            vcf_stats_dict = True # 默认返回值
            
            if out_ext == 'vcf' and out_file.exists():
                # 判断使用静态阈值还是动态阈值模型
                if not getattr(self.args, 'disable_dynamic_vcf', False):
                    if mean_depth < 50:
                        dyn_dp, dyn_freq, dyn_saf, dyn_sar = 5, 0.8, 1, 1
                        depth_level = "Low (<50x)"
                    elif mean_depth < 1000:
                        dyn_dp, dyn_freq, dyn_saf, dyn_sar = 15, 0.5, 2, 2
                        depth_level = "Medium (50x-1000x)"
                    else:
                        dyn_dp, dyn_freq, dyn_saf, dyn_sar = 100, 0.5, 10, 10
                        depth_level = "High (>1000x)"
                    
                    log_block.append(f"🧠 启用动态自适应过滤策略 [{depth_level}]: DP>={dyn_dp}, FREQ>{dyn_freq}, SAF/SAR>={dyn_saf}")
                else:
                    dyn_dp = self.args.vcf_min_dp
                    dyn_freq = self.args.vcf_min_freq
                    dyn_saf = self.args.vcf_min_saf
                    dyn_sar = self.args.vcf_min_sar
                    log_block.append(f"🔒 使用静态强制过滤策略: DP>={dyn_dp}, FREQ>{dyn_freq}, SAF/SAR>={dyn_saf}")

                # 兼容不同出图工具的标签格式差异
                if caller == 'lofreq':
                    # LoFreq 直接使用 AF 字段替代 Freebayes 的 (AO/DP)
                    filter_expr = (f"QUAL > {self.args.vcf_min_qual} && INFO/DP >= {dyn_dp} && AF > {dyn_freq}")
                    log_block.append(f"🔧 检测到 LoFreq 输出，自动切换过滤底层表达语法 (AF替代AO/DP)")
                else:
                    filter_expr = (f"QUAL > {self.args.vcf_min_qual} && INFO/MQM > {self.args.vcf_min_mqm} && "
                                   f"INFO/DP >= {dyn_dp} && INFO/SAF >= {dyn_saf} && "
                                   f"INFO/SAR >= {dyn_sar} && (INFO/AO / INFO/DP) > {dyn_freq}")

                soft_flt_vcf = virus_dir / f"{sample}.{folder_name}.soft_filter.vcf"
                cmd_soft = f"bcftools filter --no-version -s FLTER -i '{filter_expr}' -Ov -o '{soft_flt_vcf}' '{out_file}'"
                cmd_hard = f"bcftools filter -i 'FILTER==\"PASS\"' --no-version -Ov -o '{clean_vcf}' '{soft_flt_vcf}'"
                
                try:
                    subprocess.run(cmd_soft, shell=True, check=True, stderr=subprocess.PIPE)
                    subprocess.run(cmd_hard, shell=True, check=True, stderr=subprocess.PIPE)
                    ...
                    log_block.append(f"✅ VCF质量过滤完成 (过滤前可疑突变数: {raw_stats['sites']} -> 硬过滤后 PASS 突变数: {clean_stats['sites']})")
                    if soft_flt_vcf.exists(): soft_flt_vcf.unlink()
                except subprocess.CalledProcessError as e:
                    err_msg = e.stderr.decode() if isinstance(e.stderr, bytes) else (e.stderr or str(e))
                    log_block.append(f"⚠️ VCF 过滤跳过 (容错处理，原因多为非Freebayes输出导致INFO标签不一致):\n{err_msg}")

            self.write_sample_log(sample, "\n".join(log_block))
            return vcf_stats_dict 
    ...
    def run_variants_calling(self, best_results):
        ...
        with Timer("核心代表株变异检测与过滤"):
            ...
            # 将测序深度随任务包传入，用以激发子进程中的智能动态阈值质控
            tasks = [(r['Sample'], r['Virus'], r.get('taxonomy', 'Unannotated'), r['MeanDepth']) for r in best_results]
            success_count = 0
            all_vcf_stats = []
                    ...
                    for future in as_completed(futures):
                        ...
                        if res: 
                            ...
                            if isinstance(res, dict):
                                all_vcf_stats.append(res)
                        pbar.update(1)
            
            logger.info(f"✅ 成功归档了 {success_count} 个按分类学命名的核心病毒变异结果 (VCF/TSV)")
            
            if all_vcf_stats:
                stats_df = pd.DataFrame(all_vcf_stats)
                stats_csv = self.output_dir / 'summary' / 'vcf_filter_stats_summary.tsv'
                stats_df.to_csv(stats_csv, sep='\t', index=False)
                logger.info(f"📊 变异 VCF 过滤前后的对比全貌汇总已保存至: {stats_csv}")

    def _run_single_consensus(self, task):
        """将代表序列替换成该实际检测样本碱基特性的共识序列"""
    ...
    def generate_report(self, df_best, total_raw_records, total_filtered_records):
        """生成全局系统级分析综合报告，并写出至本地 summary 目录"""
        ...
        with open(report_file, 'w') as f:
            ...
            if getattr(self.args, 'rvdb_taxon', None): 
                f.write(f"本地注释文件: {self.args.rvdb_taxon}\n")
            if getattr(self.args, 'email', None): 
                f.write(f"开启 NCBI API 补全 (绑定邮箱: {self.args.email})\n")
            ...
            f.write(f"Reads提取状态: {'已提取阳性样本病毒序列 (存于 reads 下)' if getattr(self.args, 'extract_reads', False) else '未启用'}\n")
            f.write(f"共识提取状态: {'已针对核心代表株生成 (存于 consensus 下)' if getattr(self.args, 'consensus', False) else '未启用'}\n")
            f.write(f"变异检测状态: {'已使用 ' + self.args.variant_caller + ' 进行检测并执行动态硬过滤 (存于 variants 下)' if getattr(self.args, 'call_variants', False) else '未启用'}\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description='病毒分析终极版 (带断点续跑、三级过滤、动态变异质控、Reads分离与共识提取)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
核心增强功能说明:
  --resume       : 开启断点续跑。将自动跳过已完成的比对、深度计算、序列提取、共识提取和变异检测。
  --extract_reads: 将阳性样本中比对到特定病毒的 Reads 单独分离压缩出来。
  --consensus    : 基于去重最优代表序列生成针对该样本变异特性的共识FASTA序列。
  --call_variants: 开启病原体的变异检测，并内置基于测序深度的全自动 bcftools 质控硬过滤，极大降低假阳性率。
        """
    )
    ...
    vcf_group = parser.add_argument_group('变异 VCF 过滤参数 (当 --call_variants 为 VCF 输出时生效)')
    vcf_group.add_argument('--disable_dynamic_vcf', action='store_true', help='禁用基于 MeanDepth 的动态过滤模型，强制使用下方固定硬性阈值')
    vcf_group.add_argument('--vcf_min_dp', type=int, default=100, help='变异过滤: 最小绝对覆盖深度 [默认: 100]')
    vcf_group.add_argument('--vcf_min_saf', type=int, default=10, help='变异过滤: 正链最低 reads 支持数 [默认: 10]')
    vcf_group.add_argument('--vcf_min_sar', type=int, default=10, help='变异过滤: 负链最低 reads 支持数 [默认: 10]')
    vcf_group.add_argument('--vcf_min_freq', type=float, default=0.5, help='变异过滤: 最低突变频率 [默认: 0.5]')
    vcf_group.add_argument('--vcf_min_qual', type=float, default=20.0, help='变异过滤: 最低整体质量值 QUAL [默认: 20]')
    vcf_group.add_argument('--vcf_min_mqm', type=float, default=30.0, help='变异过滤: 最低平均比对质量 MQM [默认: 30]')

    filter_group = parser.add_argument_group('宏观群落丰度定量过滤参数')
    ...
    parallel_group = parser.add_argument_group('硬件与并行处理参数')
    parallel_group.add_argument('--parallel', action='store_true', help='并行处理不同的样本数据包')
    parallel_group.add_argument('--parallel_jobs', type=int, help='允许同时处理的样本并行任务数（如果不指定，自动基于当前系统情况计算）')
    parallel_group.add_argument('--threads', type=int, default=4, help='系统调用的总线程数[默认: 4]')
    
    control_group = parser.add_argument_group('兼容与调试控制参数')
    control_group.add_argument('--skip_alignment', action='store_true', help='彻底跳过比对步骤 (若使用 --resume 且 BAM 存在则自动触发)')
    control_group.add_argument('--skip_depth', action='store_true', help='彻底跳过深度计算步骤 (若使用 --resume 则自动触发)')
    control_group.add_argument('--format', type=str, choices=['csv', 'tsv'], default='tsv', help='汇总表输出文件格式[默认: tsv]')
    
    other_group = parser.add_argument_group('其他参数')
    other_group.add_argument('--verbose', action='store_true', help='输出极其详细的 DEBUG 级别底层工具调用日志')
    
    args = parser.parse_args()
    
    # 基础容错校验
    if not getattr(args, 'input_dir', None) and not getattr(args, 'sample_list', None): 
        parser.error("必须提供 --input_dir 或 --sample_list 参数，告诉程序数据在哪里")
    if not os.path.exists(args.reference): 
        parser.error(f"严重错误：参考基因组文件不存在或路径错误: {args.reference}")
    if getattr(args, 'ref_gff', None) and not os.path.exists(args.ref_gff): 
        parser.error(f"指定的 GFF 注释文件不存在: {args.ref_gff}")
    if getattr(args, 'input_dir', None) and not os.path.exists(args.input_dir): 
        parser.error(f"输入测序数据目录不存在: {args.input_dir}")
