...
Stages:
  1. detect    - Rapid quantification (batch_virus_depth40.py)
  ...
  8. similarity - Full-length similarity panorama (virus_auto_pipeline3.py)
...
def main():
    ...
    g.add_argument("--resume", action="store_true", help="Resume from checkpoints")
    g.add_argument("--force", action="store_true", help="Force re-run, ignore checkpoints")
    ...
    # ---- Stage 1: Detect ----
    g = parser.add_argument_group("Stage 1: Detection (batch_virus_depth40)")
    ...
    # ---- Stage 8: Similarity ----
    g = parser.add_argument_group("Stage 8: Similarity Panorama (virus_auto_pipeline3)")
    ...
    g.add_argument("--virema_script", default="~/biosoft/virus/ViReMa_0.29/ViReMa.py", help="Path to ViReMa.py")
    ...
    if args.force:
        log.info("  Mode:   FORCE (no resume)")
    elif args.resume:
        log.info("  Mode:   RESUME (skip completed)")
    log.info("=" * 55)
    ...
    if args.dry_run:
        ...
        if s in ("all", "detect"):   log.info("  [1/10] Detect:   batch_virus_depth40.py")
        ...
        if s in ("all", "similarity"): log.info("  [8/10] Similarity: virus_auto_pipeline3.py")
    ...
    if args.filter or args.stage == "filter":
        if best_summary.exists():
            ...
            filter_parts = [
                f"python {script_dir / 'filter_summary.py'}",
                f"-i {best_summary}",
                f"-o {high_conf}",
                f"-c {args.filter_cov}",
    ...
    if args.stage in ("all", "extract"):
        ...
        if full_dir.exists():
            parts = [
                f"python {script_dir / 'extract_full_fasta.py'}",
                f"--dir {full_dir}",
    ...
    if args.stage in ("all", "post"):
        ...
        if not summary_for_post.exists():
            ...
            virus_list = df[acc_col].dropna().unique()

            if len(virus_list) == 0:
                log.warning("  No viruses to analyze")
            else:
                log.info("  Viruses to process: %d", len(virus_list))
                ...
                with ThreadPoolExecutor(max_workers=min(len(virus_list), args.jobs)) as ex:
                    futures = {ex.submit(process_one_virus, v): v for v in virus_list}
    ...
    if args.stage in ("all", "capheine"):
        ...
        if (not cap_ref or not cap_unaligned) and gb_dir.exists():
            ...
            for _acc in _targets:
                ...
                # Extract CDS from GB
                run(f"python {script_dir / 'gbk_extractor.py'} "
    ...
    if args.stage in ("all", "similarity"):
        ...
        if not consensus_base.exists():
            ...
            for vdir in consensus_base.iterdir():
                ...
                flat_dir = Path(tempfile.mkdtemp(prefix=f"sim_{vname}_"))
                ...
                parts = [
                    f"python {script_dir / 'virus_auto_pipeline3.py'}",
                    f"-i {flat_dir}",
                    f"-g {sim_ref}",
                    f"-o {vout}",
