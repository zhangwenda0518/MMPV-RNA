    ...
    if not args.skip_blast:
        if args.online:
            ...
            if not args.db or not args.hmmer_db:
                ...
                sys.exit(1)
            diamond_out = str(out_dir / 'diamond_hits.txt')
            hmmer_out = str(out_dir / 'hmmer_hits.txt')
            run_diamond(hypotheticals, args.db, diamond_out, args.threads,
                        args.evalue, args.diamond_bin)
            run_hmmscan(hypotheticals, args.hmmer_db, hmmer_out, args.threads,
                        args.evalue, args.hmmer_bin)
            # 合并: diamond 优先, 补 hmmscan
            _merge_blast_files(diamond_out, hmmer_out, blast_file)
